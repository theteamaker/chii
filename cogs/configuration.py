...
class Configuration(commands.Cog):
    ...
    async def set_prefix(self, ctx, *args):
        ...
        if re.match(r"^.{,5}$", args[0]) is None:
            await ctx.send(
                "**Error:** The specified prefix for this bot must be *at most* 5 characters, and contain no spaces."
            )
            return

        try:
            servers_db.upsert(
                dict(server_id=int(ctx.guild.id), prefix=args[0]), ["server_id"]
            )
            await ctx.send(
...
async def limit_safe(ctx):
    ...
    if count is not None:
        if count["total"] >= 3500000:
            await ctx.send(
                "The maximum character limit has been reached! Please consult the bot creator."
            )
            return False
        else:
            return True
